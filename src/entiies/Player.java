...
/**
 * Klasse som håndterer en bevegelig spiller
...
/**
 * Metode for å bevege seg over et gitt terreng
 ...
 */
	public void move(Terrain terrain){
		...
		// Finner terrengh�yden til popsisjonen vi er i n�. 
		float terrainHeight = terrain.getHeightOfTerrain(super.getPosition().x, super.getPosition().z);
		
		// Lager en ny vektor av bevegelsesdistansene. 
		Vector3f movingDistance = new Vector3f(dx, dy, dz);
		
		// Legger de til den n�v�rende posisjonen for � finne punktet vi �nsker � g� til.
		...
		// Hvis vi ikke kommer til � krysse noe
		if(!isIntersecting){
		...
		// Hvis vi kommer til � krysse ovenfra etter et hopp (er i lufta)
		}else if(newPosition.y < intersectingBox.getMax().y && this.getPosition().y > intersectingBox.getMax().y){

				super.increasePosition(dx, 0, dz);
				super.getPosition().y = intersectingBox.getMax().y; // Flytt spillern til toppen av boksen, s� den ikke krysser med er akkurat p� toppen.
				isInAir = false;
				upwardsSpeed = 0;
		
		// Hvis vi kommer til � krysse og er i lufta, men ikke er over objektet, da er vi p� siden.
		}else if(isInAir){
			 if(super.getPosition().y < terrainHeight){
		...
		// Hvis vi kommer til � krysse, men kommer ikke opp� objektet
		}else if(this.getPosition().y < intersectingBox.getMax().y){
			// Gj�r ingenting
			
		// Ellers er vi opp� objektet, og skal kun bevege oss i z og x aksen for ikke � krysse med objektet. 
		// Dette hindrer spillern fra � falle mot bakken og vi kan bevege oss p� objektet.
		}else{
			super.increasePosition(dx, 0, dz);
		}
